...
// if everything is ok, try to upload file
    ...
    echo basename( $_FILES["fileToUpload"]["name"]);
		rename ($target_dir . basename( $_FILES["fileToUpload"]["name"]), $target_dir . $_GET['imgid'] . '.' . $imageFileType);

		$target_file_n = $_GET['imgid'] . '.' . $imageFileType;
    //echo '</br>'.$target_file_n;
		//--- update data in table employee_tb
		$sql = " UPDATE  `tbl_typecolor` SET  ";
		$sql .= "`TCOL_PHOTO` =  '{$target_file_n}', ";
		$sql .= "`TCOL_UPDATEBY` ='{$name}', ";
		$sql .= "`TCOL_UPDATETIME` = NOW() ";
		$sql .= " WHERE `TCOL_ID` ='".$_GET['imgid']."';";
